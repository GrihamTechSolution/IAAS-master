...
export class ContentHubSingleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe(data => {
      if (data['id']) {
        this.articleCategoryService.getArticleByID(data['id']).subscribe(art => {
          this.article = art;

          this.articleCategoryService.getArticles().subscribe(
            articles => {
              this.recentPost = [];
              articles = articles.filter(item => item.status == 1 && item.id != data['id']).sort((a1, a2) => a2.id - a1.id);
              /// Please, be elegant!
          ...
          if (this.currentUserID) {
            this.favouriteBlogService.getFavouriteBlogForUser(this.currentUserID).subscribe(favs => {
              this.favouriteBlogs = favs;
              let result = this.favouriteBlogs.filter(e => e.articleID == this.article.id);
              if (result.length > 0) {
  ...
  saveToFavourites(){
    ...
    this.favouriteBlogService.insertFavouriteBlog(favouriteBlog).subscribe(data => {
      if (data.status == 0){
        ...
        this.notifier.notify('error', 'Error while adding article to Favourites');
        this.loadingSpinner.hide();
      }
    })
  }
  ...
  removeFromFavourites(){
    this.loadingSpinner.show(); 
    let id = this.favouriteBlogs.filter(e => e.articleID == this.article.id)[0].id;
