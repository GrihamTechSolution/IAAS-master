...
export class AddEditEventsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (currentUrl.includes('addeditprojects')) {
  ...
  addEditArticle() {
    if (this.edit) {
      this.eventService
        .updateEvent(this.eventId, this.events)
        .subscribe(data => {
          if (data.success) {
            // this.notifier.notify('info', data.message);
            if (this.isProjects) {
            this.notifier.notify('info', 'Project updated successfully!');

              this.router.navigateByUrl('/administration/projects');
            } else {
            this.notifier.notify('info', 'Events updated successfully!');
      ...
      this.events.type = this.isProjects ? 'project' : 'event';
      this.eventService.insertEvents(this.events).subscribe(data => {
        if (data.success) {
          // this.notifier.notify("info", data.message);
          if (this.isProjects) {
            this.notifier.notify('info', 'Project added successfully!');

            this.router.navigateByUrl('/administration/projects');
          } else {
            this.notifier.notify('info', 'Events added successfully!');
  ...
  validateInput() {
    if (!this.events.title || !this.events.content || !this.events.status) {
