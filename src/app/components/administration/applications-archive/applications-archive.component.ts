...
@Component({
  selector: 'app-applications-archive',
  ...
  ngOnInit(): void {
    ...
    forkJoin([internshipsObs, applicationsObs, userObs]).subscribe(data => {
      ...
      switch(user.userTypeID) {
        ...
        this.applications = data[1].filter(a => a.status && (a.status == 1 || a.status == 2));
        this.helperApplications = data[1].filter(a => a.status && (a.status == 1 || a.status == 2));
          this.loadingSpinner.hide();
          break;
