...
@Component({
  selector: 'app-addeditarticle',
  templateUrl: './addeditarticle.component.html',
  styleUrls: ['./addeditarticle.component.css']
})
    ...
    if (this.edit) {
      this.articleService.updateArticle(this.article).subscribe(data => {
        if (data.status == 0) {
          ...
          this.router.navigateByUrl('/administration/articles');
        }
        else {
